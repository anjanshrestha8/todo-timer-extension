...
function activate(context) {
    ...
    const exportTodosCommand = vscode.commands.registerCommand("timer-todo.exportTodos", async () => {
        const categories = todoProvider._getCategories();
        const options = [
            { label: "All Categories", id: "all" },
            ...categories.map((c) => ({ label: c.name, id: c.id })),
        ];
        const pick = await vscode.window.showQuickPick(options.map((o) => o.label), { placeHolder: "Export which category?" });
        if (!pick) {
            return;
        }
    ...
    const importTodosCommand = vscode.commands.registerCommand("timer-todo.importTodos", async () => {
